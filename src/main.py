...
def getURLFor(folderName):
    baseURL = "ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/phase3/data"
    return baseURL+"/"+str(folderName)+"/alignment"+str(folderName)+".mapped.ILLUMINA.bwa.GBR.low_coverage.20120522.bam"

def ftpGetURLs(count=-1):
    urls = []

    baseURL = "ftp.1000genomes.ebi.ac.uk"
    path = "vol1/ftp/phase3/data/"
    ...
    for subDir in subdirectories:
        ...
        try:
            ...
            file = files[0]  # TODO: pristi rok MBG (zvysil sa pocet genomov na 5000?)
            urls.append((subDir, "ftp://"+baseURL+f.pwd()+"/"+file))
            if counter == count:
                break
        except:
            sys.stderr.write("Error in " + subDir + " Vid foto Mira Paveleka \n")
            pass
        finally:
            if f.pwd() != mainDict:
                f.cwd(mainDict)
